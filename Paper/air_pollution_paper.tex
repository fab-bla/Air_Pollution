...
Depending on which social insurance system a patient is covered under - more on this in the section \ref{orga} - outpatient stays are partially covered, but in general chronic diseases are partly covered by public expenditures. And so environmental pollution affects the public budget of health care. 
	...
	When compared to the original paper that we had intended to replicate by \textcite{zeng2019does} we noticed a few issues afterwards. 
	...
	One result that seems odd at the first glance is that in the SLX model only urban, but not rural disposable income per capita is significant (and vice versa in the fGLS model). However, a plausible explanation emerges from the theory: After the strict regulations and controls based on the hukou system were relaxed under Mao Zhedong, it was now possible for rural residents to settle in urban regions and take up work. However, life as a rural migrant in the city is still marked by great inequalities, including in the health care system. In order to change one's residence, there are not only financial burdens, but also the "surrender" of houses and land in the countryside. For this reason, many rural residents choose not to re-register, preferring to remain uninsured in the city, and in fact have to pay out of their own pockets. Since we do not have data on private health care expenditures, this remains only a hypothesis.
	\newpage
